...
# 
# 
# Если представить нашу модель в виде нейросети, то первый слой будет полносвязным $W \times d$, на который умножается входной вектор с единичкой на месте, соответсвующией индексу слова в словаре, таким образом получаем тот самый искомый эмбендинг нашего слова.
# Этот эмбендинг мы умножаем на матрицу контекстов $W\_c$, применением к которому софтмакса мы получаем вероятности попадания других слов в контекст нашего, которые и оптимизируем.
# 
# 
...
# 
# От этой функции авторы статьи требуют в том числе равенство нулю в нуле - именно поэтому нам не нужно рассматривать нулевые матричные элементы, градиент при их рассмотрении 0, но если бы мы наплевали на это требование и все же рассматривали их, сложность обучения многократно возросла бы, т.к. теряется основная фишка этой модели - тренироваться только на совместных вхождениях пар слов, а не по всевозможным парам слов. Тестировать такую модель - все равно, что не приближать софтмакс в w2v.
